        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.35, "y": 0.15, "w": 0.3, "h": 0.35, "name": "🖥️ Monitor"},
                    {"x": 0.25, "y": 0.55, "w": 0.5, "h": 0.25, "name": "⌨️ Keyboard"},
                    {"x": 0.65, "y": 0.6, "w": 0.2, "h": 0.25, "name": "🖱️ Mouse"},
                    {"x": 0.55, "y": 0.25, "w": 0.25, "h": 0.3, "name": "💾 CPU"},
                ]
            },
            {
                "path": "images/cpu.jpg", 
                ...
                "hotspots": [
                    {"x": 0.3, "y": 0.2, "w": 0.4, "h": 0.6, "name": "🧠 CPU Chip"},
                    {"x": 0.75, "y": 0.3, "w": 0.2, "h": 0.4, "name": "📦 Processor"},
                ]
            },
        ]
    },
        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.2, "y": 0.3, "w": 0.3, "h": 0.4, "name": "📁 Folder"},
                    {"x": 0.6, "y": 0.3, "w": 0.3, "h": 0.4, "name": "📄 File"},
                ]
            }
        ]
    },
        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.1, "y": 0.1, "w": 0.8, "h": 0.8, "name": "💻 Operating System"},
                ]
            }
        ]
    },
        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.2, "y": 0.2, "w": 0.6, "h": 0.6, "name": "📝 Text Editor"},
                ]
            }
        ]
    },
        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.3, "y": 0.3, "w": 0.4, "h": 0.4, "name": "🛡️ Protection"},
                ]
            }
        ]
    },
        ...
        "images": [
            {
                ...
                "hotspots": [
                    {"x": 0.2, "y": 0.2, "w": 0.6, "h": 0.6, "name": "🔢 Numbers"},
                ]
            }
        ]
    },
...
class HoverImageLabel(QLabel):
    """Image label with hotspot detection for hover tooltips"""
    ...
    def set_hotspots(self, hotspots):
        """Set the hotspots for this image"""
        self._hotspots = hotspots if hotspots else []
    
    def mouseMoveEvent(self, event):
        """Detect when mouse is over a hotspot"""
        ...
        # Get actual pixmap size (scaled)
        pix_w = pixmap.width()
        pix_h = pixmap.height()
        
        # Calculate scaling ratio
        scale_x = label_w / pix_w if pix_w > 0 else 1
        scale_y = label_h / pix_h if pix_h > 0 else 1
        
        # Mouse position relative to label
        mouse_x = event.pos().x() / label_w if label_w > 0 else 0
        mouse_y = event.pos().y() / label_h if label_h > 0 else 0
        
        # Check if mouse is over any hotspot
        found = False
        for hotspot in self._hotspots:
            hs_x = hotspot.get("x", 0)
            hs_y = hotspot.get("y", 0)
            hs_w = hotspot.get("w", 0.1)
            hs_h = hotspot.get("h", 0.1)
            
            # Check if mouse is within hotspot bounds
            if (hs_x <= mouse_x <= hs_x + hs_w and 
                hs_y <= mouse_y <= hs_y + hs_h):
                self.setToolTip(hotspot.get("name", ""))
                found = True
                break
    ...
    def _display(self):
        ...
        current_image_data = self._images_data[self._idx]
        current_content = current_image_data.get("content", "No description available")
        self._desc_lbl.setText(current_content)
        
        # Set hotspots for this image
        hotspots = current_image_data.get("hotspots", [])
        self._img_lbl.set_hotspots(hotspots)
